...
def test_net_init(mocker: MockerFixture):
    ...
    net = Net()
    pytest.assume(mock_super_init.assert_called_once(), "Expected super init to be called once")
...
def test_net_forward(mocker: MockerFixture):
    ...
    pytest.assume(mock_relu.called, "Expected relu to be called")
    pytest.assume(mock_log_softmax.called, "Expected log softmax to be called")
...
def test_predict(mocker: MockerFixture):
    ...
    pytest.assume(response.status_code == 200, "Expected status code 200")
    pytest.assume('prediction' in response.json(), "Expected 'prediction' in response")
